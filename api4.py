...
def get_lowest_frequency_word():
    ...
    return row
def search_youtube_videos(search_query, max_results=1):
...
def add_keyword(repeat,search_query):
    ...
    for video_id in video_ids:
        ...
        comments = get_video_comments(video_id,"AIzaSyAVnteL9JlOKZwa0cUk52PgpuqVYi7rZZQ",(0,5))
        ...
        if sample.status_code != 500:
            for res in sample:
                if res != []:
                    ...
                    ''', (res,))
                    conn.commit()
                    print("욕설 단어 저장됨 또는 cnt 증가:", sample)
